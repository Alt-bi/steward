...
## 2.37.1 — измерено, а не выведено: страница называет лот дважды

Начал с гипотезы и записываю её вместе с тем, чем она кончилась, потому что
второе полезнее первого.

**Гипотеза.** 2.37.0 берёт assetid из аргументов кнопки отмены, а
`docs/market-shapes.md` прямо говорит, что hover-блок лежит не в строках, а в
страничном `<script>` — и этот скрипт 2.37.0 не читал. Отсюда вывод: все строки
выходят «слепыми», и одна строчка `listings.some(l => !l.assetid)` отдаёт прогон
в `fetchMyListings`, который по устройству идёт до последней страницы аккаунта.
Область скана осталась бы страницей, а цена — весь аккаунт.

**Замер на живой странице (10 лотов, аккаунт на 700+).** Кнопка отмены
совпала на 10 строках из 10; hover-скрипт дал 10 ссылок из 10; все 10 лотов с
assetid, ценой и контекстом. То есть на этой вёрстке путь 2.37.0 работал, и
догруз не запускался ни разу. Гипотеза была правдоподобной и неверной — ровно
то, для чего в этом репозитории заведено правило мерить, а не рассуждать.

Что осталось в коде, уже как страховка, а не как лечение:

- **`hoverBlobOnPage()` вернулся** и читает `document.scripts`;
  `listingsFromDom` принимает блоб аргументом, а не выковыривает его из хоста
  строк. Кнопка отмены — по-прежнему первый источник. Теперь их два, и оба
  измерены как присутствующие; раньше рабочим был ровно один, и какой именно —
  никто не проверял.
- **Догруз перестал быть обходом аккаунта.** `fetchAssetRefsFor(ids)` идёт по
  `mylistings` только пока названные строки не получат свои assetid;
  `fetchMyListings` — тот же обход без условия остановки, для случая, когда
  полный список наших лотов и есть вопрос. На той единственной странице, где
  страница промолчит, разница между «два запроса» и «семь» — это разница между
  задержкой и штрафным ящиком, который 2.35.0 разбирал неделю.
- **Скан говорит, сколько строк осталось без assetid**, вместо того чтобы
  оставлять это объяснять каждому ряду по отдельности.

**Проверка «ничья» не умела говорить «нет».** Найдено при чтении того же пути.
`competitorFromScan` разрешает подрезать лот с ценой ровно как у нас, только
если книга сама назвала владельца каждой строки (`flagged`) — иначе это может
быть наш лот с другой страницы. `flagged` — это `rows.every(r => r.mine !==
undefined)`, а `bookListingFrom` писал `mine: row.bMine === true`, превращая
отсутствующее поле в `false`: `flagged` не мог оказаться ложным никогда.
Сегодня безвредно — измерено, `QueryListingsForItem` шлёт `bMine` во всех
двадцати строках. Перестанет быть безвредным в тот день, когда перестанет, и
скоуп по странице поднял ставку: свои listing id теперь одна страница аккаунта,
а не весь он, так что флаг книги — почти всё, что стоит между прогоном и торгом
с самим собой. `mine` стал `boolean | undefined`, ставится только из настоящего
boolean. «Не `true`» по-прежнему значит «не наш» везде, где на этом основано
действие.

**И зонд, чтобы следующее «не работает» стоило один вечер.** `npm run probe`
собирает боевые читатели в `.probe/market.js`; вставленный в консоль на /market,
он печатает, что они увидели — нарисованные строки, разобранные hover-ссылки,
совпавшие кнопки отмены, лоты с assetid — и вердикт одной строкой. Так измерялся
`docs/market-shapes.md`, и именно он за один запуск опроверг гипотезу, под
которую я уже написал код. Инструмент дешевле, чем ещё один правдоподобный
вывод.

Тестов 688. Каждая правка проверена против своего бага: без чтения скрипта
рыночный тест падает на запросе `/market/mylistings`, которого быть не должно;
без условия остановки обход платит три запроса там, где хватает двух; с `mine`,
свёрнутым обратно в голый boolean, книга, не назвавшая никого, читается как
книга, назвавшая всех.


## 2.37.2 — один отказ книги выключал всю сеть

Отчёт: «Steam дважды прислал веб-страницу вместо данных книги лотов… Посчитано
по рыночному минимуму: 0 из 10. Запросов 3.»

Ноль из десяти — вот что здесь неправильно, а не отказ книги. На отказ книги у
прогона есть готовый ответ: 2.36.0 специально завёл спасательный проход —
`priceoverview` и `search` живы, книгой они не являются, и именно они считают
минимум, когда книга молчит. Он не сделал ни одного запроса.

Механизм, целиком:

1. `fetchRaw` с 2.35.0 сообщает разметку вместо JSON как `rate_limited` — чтобы
   темп падал по дороге ко второй стадии троттлинга. Разумно.
2. `BLOCK_AFTER = 1`: один `rate_limited` поднимает `state.blocked`. Тоже
   разумно — это про 429, после которого стучаться нельзя.
3. Но `blocked` — глобальный, и `acquire` проверяет его первым **для любого
   kind**. Первая же страница вместо книги закрывала сеть целиком.

То есть спасательный проход был выключен ровно тем событием, ради которого его
писали. «Запросов 3» — это два запроса книги (два воркера ушли одновременно) и
третий, посчитанный, но так и не отправленный: `requests += 1` стоит до вызова,
а `waitForSlot` бросает `blocked` ещё до сети.

Починка — в словаре, а не в пороге. У исхода появилось имя `wrong_shape`:
разметка там, где ждут JSON, это **отказ эндпоинта**, а не лимит на IP. Он
получает всё, что 429 даёт виду — половину темпа, обнулённый бёрст, паузу, —
и ничего из того, что 429 даёт аккаунту: ни удара по счётчику, ни
предохранителя, ни глобального кулдауна. Пауза теперь тоже своя, `KindState.
cooldownUntil`: молчит книга — молчит книга, а не `priceoverview`. Настоящий
бан (429, sorry-page по `isSteamRateLimitBody`) как закрывал всё, так и
закрывает.

Заодно: `fetch` идёт по 302 молча, поэтому «пришла веб-страница» до сих пор
покрывало две противоположные ситуации — эндпоинт нам отказывает и эндпоинта по
этому адресу для этой сессии просто нет. Лечатся они по-разному, а различает их
один бит, `res.redirected`, который мы выбрасывали. Теперь он в тексте ошибки:
«перенаправлено на /market/ — Сообщество Steam…».

И статус перестал сваливать вину на книгу: если спасательный проход тоже
остановлен, он говорит об этом отдельно — и отдельно же говорит, если его *даже
не спросили*, потому что это уже наш предохранитель, а не отказ Steam.

Почему это дожило до пользователя: все тесты рыночной вкладки подменяют
планировщик через `setAcquire`, то есть выключают ровно тот механизм, который
ломался. Новый тест `prices through the real governor after the book answers
with a page` гоняет настоящий: книга отдаёт домашнюю страницу, `priceoverview`
отвечает, и прогон обязан закончиться «2 из 2». С возвращённым `rate_limited`
он падает — как и падал у пользователя.

Тестов 694 (+6): три на планировщик (сосед не глохнет, темп всё-таки падает,
пауза кончается), два на сеть (`wrong_shape` вместо бана, редирект назван
редиректом) и сквозной выше. Шим `fetch` в тестах научился редиректам.


## 2.38.0 — одна цель, и она не наша собственная

По просьбе: вкладка репрайса больше не умеет ничего, кроме одного — сравнить
нашу цену с минимальной чужой и встать на копейку ниже. Уровни «средняя за
неделю/месяц/год» убраны вместе с кнопкой «История продаж», выпадающим списком
«Цена», `planAtLevel`, `state.stats` и `state.level`. `core/levels.ts`,
`pricehistory` и `history-load` остались — на них живут вкладки инвентаря и
предмета, там история считает другое и не мешает.

Ушёл заодно самый медленный запрос Steam (шесть в минуту, бан на часы) — из
этой вкладки его теперь физически не сделать.

**Главное — вторая половина просьбы: «важно чтобы мы не брали наши предметы с
минимальной ценой».** Тут была настоящая дыра, и её открыло само же
постраничное сканирование. Книга лотов помечает наши строки (`bMine`) и вопроса
не возникает, но книга у пользователя отказывает — считается по
`priceoverview`, а это голое число. Сравнивали его с `group.ourLow`, то есть с
самым дешёвым нашим лотом **на этой странице**. Аккаунт на 700 лотов рисует
десять: минимум ниже страничного — это ровно так же часто наш собственный лот со
страницы, на которую мы не смотрим. Подрезали бы себя, по копейке за скан,
вечно.

Правило теперь одно и записано в типе: `theirs === false` — это «посмотрели и
не смогли определить», и против него не двигается ничего. Минимум из ценового
эндпоинта становится конкурентом только когда он ниже **каждого** нашего лота
этого предмета, а для этого у `ItemGroup` появился `ourLowAnywhere` — наш самый
дешёвый лот по всему аккаунту.

Узнаётся он одним проходом `mylistings`, и только тогда, когда от него зависит
решение: `learnOurLots()` смотрит, есть ли хоть одна группа, где мы иначе
шагнули бы под неопознанный минимум. Нет такой — прохода нет. Частичный ответ не
используется вовсе: половина наших лотов ничего не доказывает о том, чей лот
держит минимум. Сканирование как было постраничным, так и осталось — страница
решает, что мы переставляем; аккаунт отвечает на другой вопрос, чей это лот.

Тест ровно на это: страница показывает два лота, аккаунт держит третий, того же
предмета, дешевле обоих, и именно его `priceoverview` называет минимумом рынка.
Наш лот не двигается, соседний — двигается. С возвращённым сравнением по
странице тест падает.

Тестов 690.


## 2.39.0 — книга лотов жива, просто мы звонили не туда

Первый заход, где всё измерено прямо в браузере пользователя — Edge, живая
сессия, наши же парсеры. Три факта, каждый переворачивает то, что было в коде.

**1. `/market/actions?q=QueryListingsForItem` не отвечает вообще.** Не редирект,
не троттлинг: `200`, `text/html`, мегабайт домашней страницы маркета — с
заголовком загрузчика, без него и с классической AJAX-подписью. Все три
варианта, подряд. Именно это и было «Steam дважды прислал веб-страницу» в конце
каждого скана. Эндпоинт, на который 2.36.1 перевёл всю точную проверку, для
нашего контекста мёртв.

**2. Классический `/market/listings/{appid}/{hash}/render/` — жив.** Тот самый,
про который в DEVLOG написано «умер». `application/json` с первого раза и
каждый раз: `listinginfo` по id лота, цены раздельно `converted_price` +
`converted_fee`, `total_count` за окном. Наши собственные лоты в нём есть и
узнаются по listing id — то есть чужой минимум считается точно, без всяких
флагов владельца.

Заодно рухнули две легенды:

- **Никакой стены из group id.** `AK-47 | Redline (Field-Tested)` отвечает по
  `market_hash_name`, 1201 лот в книге. Весь детур через `search` за внутренним
  id существовал только потому, что его требовал мёртвый эндпоинт. Удалён:
  `GROUP_ID_APPS`, `learnGroupForItem` в скане, `forgetGroup`, параметры
  `groupName`/`hash` у книги.
- **У товарных лотов книги нет.** `Fracture Case`: `total_count: 1` и пустой
  `listinginfo` — кейсы и ключи торгуются ордербуком, а не лотами. Это ответ, а
  не отказ, и считать его отказом — значит уронить весь скан в кулдаун из-за
  предмета, который ведёт себя штатно.

**3. `count` — это белый список, а не число.** Один и тот же предмет, по одному
запросу на значение:

| count | ответ |
|-------|-------|
| 1, 10, 20, 100 | `success: true`, книга |
| 5, 11, 12, 25, 50, 75 | `success: false`, `total_count: 0`, ни строки |

А `scanWindow` возвращала `ourCount + 10`, то есть **11 для любого предмета, у
которого мы держим один лот** — почти для каждого. Мы бы спрашивали книгу на
глубину, которую Steam не отдаёт, получали пустой ответ, читали его (совершенно
правильно!) как отказ про предмет, который сами же продаём, — и через два таких
останавливали прогон, а через четыре загоняли планировщик в кулдаун. Идеально
тихий, стопроцентный отказ, неотличимый от троттлинга. Теперь окно округляется
вверх до ближайшего размера, который Steam обслуживает.

Темп: 10/мин у `listings` был подобран под мёртвый эндпоинт и стоил минуты
ожидания на десяти предметах. Тридцать запросов подряд с шагом 1.2 с не вызвали
ни одной деградации, так что теперь 20/мин — вровень с общим IP-бюджетом,
который и так остаётся настоящим потолком. `SEARCH_SAMPLE` снижен с 6 до 3:
`search/render` по хешу карточки отвечает `total_count: 0` (индекс не знает
таких имён), и шесть запросов уходили на то, чтобы выяснить это дважды.

Проверено на живом аккаунте (669 лотов, страница из 10): книга отвечает по
всем десяти, план получается осмысленный — четыре предмета «мы одни в книге,
двигать нечего», пять «переставить на копейку ниже чужого», один «уже дешевле».
Ровно то, ради чего вкладка существует.

Тестов 691.

## 2.40.0 — комиссия у пола рынка считалась не так, и половина страницы жила у пола

Второй заход с измерениями в живом Edge. Три вещи, и первая из них тихо портила
каждый второй лот на странице.

**1. Комиссия издателя не имеет пола в 1 копейку — у неё тот же пол, что и у
комиссии Steam.** Кошелёк говорит `wallet_fee_minimum: 87`, и Steam берёт
`max(цена * процент, 87)` **обоими** вычетами, а не только своим. Наша формула
брала `max(цена * 10%, 1)`. Выше пола разницы нет — потому ошибка и прожила так
долго: пять дорогих лотов страницы считались верно до копейки. А пять дешёвых —
нет. Карта у дна рынка стоит `87 + 87 + 87 = 261`, мы считали `87 + 87 + 8 = 182`.

Дальше хуже. Из неверной комиссии выводится обратная задача: «какую цену
продавца поставить, чтобы покупатель заплатил 2,60». Двоичный поиск честно
находил 158 — и Steam выставил бы этот лот за **3,32 ₽**. То есть на просьбу
подрезать на копейку расширение подняло бы цену на четверть, отчитавшись о
снижении. Проверено на ответе самого Steam: `converted_price: 87`,
`converted_fee: 174`. Сто лотов AK-47 и десять лотов аккаунта — новая формула
сходится на всех, старая на пяти из десяти.

**2. У рынка есть дно, и о нём надо говорить вслух.** `wallet_market_minimum: 87`
— меньше Steam лот не примет. Пять из десяти лотов страницы стоят ровно 2,61 ₽,
и чужой лот в книге стоит ровно столько же. Подрезать некуда: 2,60 не бывает.
Раньше план предлагал этот перенос каждый скан, и согласиться значило снять лот
и не выставить его обратно. Теперь `sellerForBuyer` не ищет ниже пола вовсе, а
план отвечает «дно рынка — дешевле Steam не примет».

**3. Пагинация «Моих лотов» оставляет hover-блок от прошлой страницы.**
Измерено: перелистнули на вторую — строки переписались через AJAX, а
`document.scripts` не изменился ни на байт. Блок по-прежнему описывает первую
страницу, все десять его лотов на экране отсутствуют, и все десять
по-прежнему разрешаются в `g_rgAssets`. Читая его как есть, страничный
читатель не дополнял строки — он **выдумывал** десять новых: лоты, которых
владелец не видит, с ценой в ноль, и их же предлагал переставлять. Теперь
hover-блок обрезается по строкам, которые страница действительно нарисовала.
Заодно измерено: строки нести assetid умеют сами — кнопка отмены есть у всех
десяти на обеих страницах, так что доход в `/market/mylistings` за этим не нужен.

Побочно проверено и записано, чтобы не искать снова: `query=` у
`/market/mylistings/render/` **игнорируется** — фильтра по названию нет, ответ
всегда весь аккаунт (669). И страницы предметов appid 753 — классические, без
`window.SSR`, с живым `g_rgWalletInfo`.

Тестов 699.

## 2.41.0 — снятый лот возвращается под другим id, а «чей минимум» спрашивают у предмета

Два измерения в живом браузере, и оба отменяют по механизму.

**1. «Снят, но НЕ выставлен» — это не задержка, это другой assetid.** Лот держал
предмет `38179473068`. После `removelisting` та же карта лежит в инвентаре как
`39042662381`, `marketable: 1`, `tradable: 1`. **Steam возвращает предмет под
новым id.** Цикл повторов был написан под медленную выдачу: подожди и спроси
тем же. Восемь раз, тридцать секунд, один и тот же честный ответ «такого
предмета в инвентаре нет» — и лот остался снятым.

Повтор теперь не переспрашивает, а ищет заново: после снятия расширение
находит предмет в инвентаре по `market_hash_name` и выставляет тот id, который
там лежит сейчас. Спрашивается это **до** первого `sellitem`, а не после его
провала — потому что провалился бы он всегда. Копии одной карты
взаимозаменяемы, но одна и та же копия не достаётся двум лотам: что уже
выставлено в этом прогоне, помечено занятым; при прочих равных берётся та,
которой в инвентаре только что не было.

**2. Обход всех 669 лотов был лишним — и вопрос надо задавать предмету.**
Чтобы отличить чужой лот у нашей цены от нашего же лота на другой странице,
скан обходил `/market/mylistings` целиком: семь запросов и три мегабайта на
каждый скан ради десяти listing id, которые действительно решают. Сократить
обход нечем — измерено: `count` упирается в 100, сколько ни проси (500, 1000 —
всё равно `pagesize: 100`), а `norender=1` выкидывает вместе с разметкой и
hover-блок, то есть ровно те id, за которыми шли.

Зато страница самого предмета — 90 КБ и один запрос — несёт блок «Мои лоты»:
наши лоты **этого предмета**, с listing id, asset id и ценой. Вопрос теперь
задаётся там, где на него отвечают: по предмету, и только по тем предметам, где
ответ вообще что-то меняет. Предмет, который уже дешевле, один в книге или
стоит на дне рынка, не спрашивают вовсе — двигать его всё равно нечем.

Ответ проверяет сам себя: если страница не назвала лот, который мы видим
своими глазами, это не меньший ответ, а другая страница (разлогинило,
редирект), и она не верится целиком. Книга при этом сохраняется целиком, так
что узнать задним числом «минимум был наш» — это пересчёт, а не второй запрос.

На странице из 10 лотов при 669 в аккаунте: было 17 запросов и ~3,3 МБ, стало
10–11 запросов и ~0,2 МБ. Строка статуса больше не носит дисклеймер про весь
аккаунт — она называет число предметов, по которым владельца выяснить не
удалось, и молчит, когда таких нет.

Побочно: `converted_steam_fee: 87` и `converted_publisher_fee: 87` в ответе
книги — третье независимое подтверждение вчерашней правки комиссии.

Тестов 704.

## 2.42.0 — панель без пяти лишних органов, и дорога до владельца выбирается по цене

**1. Убраны «только оверпрайс», «Все», «Ничего», «CSV» и «Сортировка».** Вместе
с кодом за ними: ключ сортировки, фильтр «только движимые», выгрузка таблицы и
две сохранявшихся настройки. Осталась одна строка управления — поиск.

Порядок строк теперь не выбирается, а один: сначала то, что прогон собирается
двигать, крупнейший сдвиг сверху; пропуски под ними. Из трёх вариантов сортировки
два («по названию», «дороже лот») закапывали ровно те строки, ради которых
вкладка существует, а переключатель, который никто не трогает, стоит только
ширины.

**2. Спрашивать владельца по одному предмету — дёшево не всегда.** Вчерашняя
замена обхода аккаунта на страницу предмета была измерена на странице из десяти
лотов, где помогала: один запрос вместо семи. На живом прогоне со страницей из
ста лотов оказалось «45/67» — шестьдесят семь предметов, у которых владельца
минимума выяснить не удалось, то есть шестьдесят семь запросов там, где весь
аккаунт читается за семь.

Теперь обе дороги оцениваются и берётся короткая. Сколько лотов в аккаунте,
страница пишет сама (`tabContentsMyActiveMarketListings_total`), а
`/market/mylistings` отдаёт по сотне за ответ — значит цена обхода известна до
того, как потрачен первый запрос. Больше предметов, чем страниц аккаунта, —
читаем аккаунт; меньше — спрашиваем по предметам. Страница из 10 лотов
по-прежнему стоит один запрос, страница из 100 — семь, а не шестьдесят семь.

Тестов 702.

## 2.43.0 — панель говорит, что она собирается сделать, до того как это сделает

Пять органов управления убраны в прошлый заход. Этот — про то, что осталось, и
про то, чего не хватало.

**1. Глубина сдвига теперь число, а не длина стрелки.** `111,87 → 76,70` и
`109,39 → 100,79` выглядят одинаково и одинаковыми не являются: первый — минус
треть цены. Рядом с ценой стоит `−31%`, и он окрашен: до 12% серый, до 30%
жёлтый, глубже — красный.

**2. Глубокий сдвиг возвращается неотмеченным.** Подрезают на копейку, но
копейка отсчитывается от чужой цены, и один дешёвый лот в тонкой книге
превращает перестановку в «минус 56%». Шестьдесят семь таких уходят одним
кликом, который никто не читал построчно.

Первой версией был отказ, и он оказался хуже: лот, который план не трогает и не
объясняет, — это лот, о котором владелец не узнает. Теперь строка остаётся на
виду, называет глубину, получает пунктирную красную границу и **снятую
галочку**. Порог — в настройках, по умолчанию 35%, 100 снимает ограничение.
Снятие однократное: если галочку вернули, следующий пересчёт её не тронет.

**3. Кнопка перестановки называет цену вопроса.** Каждый лот — это два
запроса записи при бюджете восемь в минуту плюс две паузы, то есть двадцать
секунд. Шестьдесят семь лотов — двадцать две минуты, и всё это время они сняты
с продажи. Диалог теперь пишет и время, и самый глубокий сдвиг в партии — тот,
до которого никто не долистал.

**4. Заголовок статуса — одна строка, оговорки под «подробнее».** Было четыре
строки текста, в которых ответ («к переносу 65 из 100») стоял первым и тонул
среди трёх уточнений. Уточнения никуда не делись — они свёрнуты.

**5. Счётчики стали фильтром.** Клик по «67 ОВЕРПРАЙС» оставляет в списке
только их, повторный клик снимает. Это тот же «только оверпрайс», который был
убран, — но он не занимает ни пикселя, потому что число рисовалось всё равно.

**6. Кнопки перестали ломаться на две строки.** «Сканировать» — во всю ширину,
остальные три под ней. Четыре равные кнопки в панели 450 px переносили каждую
подпись, и ряд читался как сломанный.

Мелочью: строка причины прижата к одной строке с многоточием и полным текстом в
подсказке — раньше она разъезжала строки по высоте и наезжала на колонку цен.
Слово «оверпрайс» из неё убрано: об этом уже говорят граница и процент, а на
освободившемся месте — «тебе будет 66,70 ₽», то есть деньги после переноса, а
не до.

Тестов 710.

## 2.43.1 — соседнее правило съело отступ под главной кнопкой

`.stw-actions-rest` просил `padding-top: 8px`, но следом стояло
`.stw-actions + .stw-actions { padding-top: 0 }` — два класса против одного,
специфичность выше, и отступ обнулялся. Широкая «Сканировать» садилась вплотную
на ряд из трёх, и блок читался как одна слипшаяся кнопка.

Отступ теперь несёт само соседнее правило: 8 px — ровно тот же `gap`, с которым
кнопки стоят друг от друга по горизонтали, так что вертикаль и горизонталь
совпадают. Дублирующие правила `.stw-actions-rest` и `.stw-resume` удалены —
они всё равно ничего не решали, а «Догрузить цены» заодно перестала прилипать
к ряду сверху.

Тестов 710.

## 2.44.0 — настроек больше нет; обход аккаунта не выбрасывается; молчаливый отказ Steam переспрашивается

**1. Главное меню перестало быть формой.** Девять полей и два флажка, и у каждого
ровно один верный ответ: пауза 2,5 с — то, что держит IP вне списка Steam;
параллельность 2 — то, под что мерился планировщик; «на копейку ниже» — вся
идея вкладки. Поле, которое можно выставить только хуже, — не настройка, а
ловушка с подписью. Всё это уехало в `FIXED_SETTINGS` и **штампуется поверх
хранилища на каждом чтении**, а не просто лежит дефолтом: восьмисекундная пауза,
оставленная старым попапом, иначе пережила бы поле, которое её записало.
Список стандарта попап рисует **из той же константы** — спрятанное значение,
которого не видно, ничем не лучше значения, которому нельзя верить.

Осталось: имя с версией (по ней видно, что перезагрузка взялась), две кнопки,
карточка состояния — вердикт, полоса запаса, счётчики — и два свёрнутых блока.
`clampSettings` удалён: писать стало нечего.

**2. Обход аккаунта хранится целиком.** `/market/mylistings` отдаёт по 100 лотов,
666 лотов — семь запросов по 6 с. Прочитав все, код оставлял те несколько
предметов, что спрашивали, и выбрасывал остальные шестьсот; вторая страница
маркета платила те же семь запросов за лоты, которые лежали в памяти минуту
назад. Теперь ответ кладётся целиком и отвечает на любой предмет на любой
странице, пока то, что мы держим, не изменится, — `forgetOurLots` гасит его
вместе с поштучным кэшем.

**3. «Ошибка при выставлении предмета на продажу».** Пожатие плечами, за которым
прячется и лимит на создание лотов, и всё, что Steam решил не называть. Ни один
шаблон его не ловил, поэтому один невнятный ответ обрывал прогон из 73 лотов на
двадцатом — 54 нетронутых. Предмет при этом всё время лежит в инвентаре, так что
переспросить стоит запросов и не стоит ничего: два повтора с длинной паузой и
переуказанием предмета, потом верим.

А когда всё-таки останавливаемся, отчёт **смотрит**, где предмет. Строка
«снят, но НЕ выставлен — предмет в инвентаре» утверждала это, ни разу не
проверив, — догадка ровно в том месте, где владелец читает факт. Теперь один
запрос инвентаря в конце неудачного прогона, и в отчёте то, что нашлось.

Тестов 715.

## 2.45.0 — вкладка «Инвентарь»: строка называет цену выставления и деньги на руки

**1. Строка перестала показывать не то число.** Было «×12 · 1 234,56 ₽»: количество
и сколько стопка стоит по рынку. Ни то, ни другое не решение. Стоимость стопки
уже стоит в счётчике сверху, а **цена, по которой лот встанет**, считалась,
использовалась и не показывалась ни разу — при «ниже минимума» или «по средней
за год» она может лежать заметно под рынком, и увидеть это можно было только в
диалоге подтверждения. Теперь в колонке цен `87,64 ₽ → 77,00 ₽` и чип глубины,
жёлтый с 12%, красный с 30% — те же правила, что на вкладке репрайса.

Во второй строке — деньги, которые придут: «3 шт · тебе будет 200,90 ₽», уже за
вычетом комиссии. Раньше это число жило только в `window.confirm`, то есть в
последнем месте, где решение ещё можно изменить, и в худшем месте, где можно
узнать новую цифру.

**2. Семь рядов управления стали четырьмя.** «продаваемые» и «с ценой» прятали
одно и то же дважды: строки, которые маркет не возьмёт, и строки, которым не
нашлась цена, — это строки, которые нельзя выставить. Ровно их считает счётчик
«к продаже», так что нажатие на счётчик и есть фильтр. «CSV» и «Сортировка»
ушли следом за своими двойниками с вкладки репрайса, «Все» и «Ничего» — тоже:
у каждой строки есть своя кнопка «продать». «Шаг» появляется только у двух
стратегий, которые его читают, а не висит серым у четырёх остальных.
«Оценить страницу» получила всю ширину, остальные две встали под ней.

**3. Вопрос называет цену вопроса.** Одна запись на копию при бюджете восемь в
минуту плюс пауза — двести копий это полчаса, и узнать об этом можно было только
из прогресс-бара, когда отвечать «нет» уже поздно. Диалог теперь пишет и время,
и самую большую уступку рынку в партии.

**4. Выставление тоже переспрашивает.** `sellitem` здесь звался напрямую, так что
дежурная отписка Steam из 2.44.0 превращала выставленную копию в строку «ошибка».
Теперь та же выдержка, что на вкладке репрайса: одна попытка по самому предмету
(снимать было нечего — «предмета нет в инвентаре» здесь приговор), и два
повтора с длинной паузой на невнятный отказ.

**5. `humanMinutes` переехал в `core/duration.ts`.** Две копии одного форматтера
уже разошлись: одна говорила «меньше минуты» там, где вторая говорила «30 с».

Тестов 722.

## 2.46.0 — «Игра», «Стратегия», «Шаг», «Штук» убраны вместе с тем, что за ними стояло

**Игра не выбирала ничего.** `scan()` читает `visibleTileRefs(document)` — плитки,
которые Steam уже нарисовал, — а безымянные дочитывает по контекстам, которые
называют сами эти плитки. Выбранная «Игра» доходила ровно до одной строки кода:
статуса «Выбрано: …». Селектор, `fillGames`, `selectedChoice`, подписка на
`hashchange` и таймер на 600 мс удалены; заодно ушла причина, по которой
тест-файл приходилось учить не выдёргивать документ из-под этого таймера.

**Стратегия зафиксирована на «по минимуму рынка».** Это единственная, которой не
нужно ничего сверх цены, уже полученной сканом: две подрезающие требуют шага,
который никто не может выбрать осмысленно, а три «по средней» — `pricehistory`,
шесть запросов в минуту, ради ответа про неделю, которая уже прошла. Вместе со
стратегией со вкладки ушли кнопка «История продаж», весь проход `loadHistory` и
`state.stats`. `strategyTarget` и остальные стратегии остались в планировщике и
под тестами — выбрать их с экрана больше нельзя.

**«Штук» стал ограждением, а не предпочтением.** Стопка в двести копий — это
двести размеренных записей; двадцать — та граница, за которой проход перестаёт
быть проходом. Значение фиксировано на 20.

`sell` теперь **штампуется поверх хранилища** так же, как остальной стандарт:
стратегия, записанная старым селектором, не должна пережить сам селектор.
Писать настройки в расширении больше некому — `saveSettings` осталась одной
дверью на случай, если какая-нибудь снова заслужит контрол.

**Строка перестала показывать стрелку между двумя одинаковыми числами.** При
«по минимуму рынка» цена выставления обычно и есть минимум; стрелка появляется
только когда обратный ход комиссии сдвинул её на копейку — то есть когда ей
есть что сказать.

Тестов 727: добавлен `test/settings.test.ts` — хранилище не перебивает стандарт
(проверено, что без штампа тест падает).

Осталось без вызовов: `src/core/csv.ts`, `src/steam/history-load.ts`,
`clampSellSettings`. Модули не трогал.


## 2.47.0 — панель перестала объяснять то, что и так написано

Проход по всем оставшимся вкладкам: тексты, кнопки, вид.

**Подпись в шапке печатала список вкладок над баром вкладок.** «инвентарь ·
предмет» серым — ровно те же слова, что и кнопки под ними, только тише. Теперь
подпись называет раздел только тогда, когда бара нет (одна вкладка — он
скрыт), и молчит, когда он есть. `Panel` больше не принимает subtitle.

**«Фильтр» над полем с плейсхолдером «поиск по названию» — подпись к
фотографии самой себя.** Убрана на всех трёх вкладках; вместе с ней ушли
`field()`, `narrowField()` и правила `.stw-field*`. Заодно выяснилось, что
селектов в расширении не осталось нигде — `.stw-select`, `.stw-num`,
`.stw-toggle` вырезаны как мёртвые.

**«Обмены» повторили путь «Инвентаря».** Сортировка (четыре порядка одних и тех
же строк) зафиксирована на «сначала тревожные» — это единственная причина, по
которой вкладка существует. Два чекбокса — «только живые» и «только с
замечаниями» — называли ровно те подмножества, которые счётчики над ними уже
считали: теперь нажатие на счётчик и есть фильтр. Ряд контролов исчез, ничего
не потеряно. Кнопка «Оценить» переименована в «Узнать цены»: она спрашивает
Steam, а не выносит оценку.

**Длинные статусы разложены на ответ и сноски.** `status()` на вкладках
«Инвентарь», «Обмены» и «Предмет» получил третий аргумент — он складывается под
«подробнее», как на «Репрайсе» с 2.41. Абзац из четырёх фактов хоронил первый;
теперь первый — заголовок, остальные под сгибом.

**Мёртвая ссылка в тексте:** «подними лимит в настройках» звало в настройки,
удалённые в 2.44. Заменено на «такой лот покупай на странице Steam».

**`.stw-ladder` не имел правила `[hidden]`,** а имел `display: flex` — то есть
пустая рамка на вкладке «Предмет» висела до первой проверки. Правило добавлено.

Вид: палитра названа переменными на `#stw-root` (раньше «синий у ховера» жил в
четырёх местах и расходился), вкладки получили подчёркивание вместо заливки,
счётчики стали карточками с зазором, кнопки — градиент и просадку под нажатием,
строки — скругление и подсветку под курсором. Подписи счётчиков переведены в
нижний регистр: «НЕ ПРОВЕРЕНО» капсом не влезало в свою четверть 440-пиксельной
панели и обрезалось. Имена в строках «Инвентаря» больше не переносятся — одна
строка, одна высота, как на «Репрайсе».

Тестов 732: добавлен `test/panel-shell.test.ts` — версия отдельным узлом,
подпись не дублирует вкладки, сгиб не переносит вчерашние сноски в новый ответ.
Проверено, что без правила про бар тест падает. `classList.toggle` в фейковом
DOM теперь возвращает состояние класса, как в браузере, а у элемента появилось
свойство `open` — `<details>` статуса им пользуется.

**Удалено то, что уже никто не звал.** `src/core/csv.ts` жил только в
собственном тесте, `src/steam/history-load.ts` — вообще ни в одном импорте,
`clampSellSettings` не вызывалась с тех пор, как настройки перестали быть
контролами, а `src/core/prefs.ts` хранил выбор сортировки, стратегии и фильтра
— всё, чего больше нет. Четыре файла и одна функция ушли; тестов стало 727
(минус пять про CSV), `content.js` — 186,8 КБ, `content.css` — 12,9 КБ.

## 2.48.0 — вкладка, которая говорит, чем всё кончилось; и «Переставить» наконец можно отменить

Разбор «чего не хватает» дал четыре пункта, и все четыре оказались про одно:
машинерия уже написана, а вопрос ею не задаётся.

**README обещал вкладку, которой нет, и отказывался от того, что есть.**
«Buy orders» описан как живая функция в трёх местах — вкладку снесли в 2.36.1.
Таблица «чего Steward не будет делать» обещала, что float недостижим, — он
достижим и достаётся с 2.22.0 одним запросом на весь экран через
`itemdynproperties`. Первый экран на GitHub врал в обе стороны сразу: скрывал
единственный козырь против SIH и рекламировал пустоту. Заодно вычищены
«sorting» и «game picker», которых нет с 2.46.

**На чужом инвентаре панель предлагала «Выставить».** Вкладка монтируется на
любой `/inventory`, а владелец определялся `ownerFromUrl`, который на vanity-URL
не может ничего и возвращал зрителя. Дальше «мой ли это инвентарь» проверялось
как «steamid равен зрителю» — то есть сравнением значения с его же собственным
дефолтом. Условие истинно на любой странице, и проверялось оно ровно в одном
месте: перед загрузкой износа. Ни `sell()`, ни `quickSell()`, ни `plan.ts` про
владельца не знали вообще. Steam такой `sellitem` отклонил бы, и отказ читался
бы как наш баг.

Починка не в проверке, а в источнике: страница называет владельца сама —
`g_rgProfileData.steamid`. Мост её проецирует, `ownerFromUrl` берёт её первой, и
`mine` теперь **никогда не истинно по догадке**. Кнопки записи на чужой странице
не выключены, а скрыты: серая «Выставить» на своей странице значит «ничего не
отмечено», и одна внешность не может честно нести оба смысла. То же правило
повторено у самого вызова — прятать кнопку это решение о рисовании, а `sellitem`
тратит настоящую сессию.

**`/market/myhistory` не упоминался в `src/` ни разу.** Семь вкладок отвечали
«сколько это стоит» и ни одна — «что из этого вышло». Новая вкладка «Продажи»
читает историю по сто записей за запрос и считает выручку, комиссию, потраченное
и что принесло больше всего.

Два отказа в парсере, оба про то, чтобы не выдумывать числа:

1. **Даты не разбираются.** Steam печатает их коротко, локализованно и без года
   («4 сен», `Sep 4`). Превратить это в timestamp — угадать локаль, а потом
   угадать, какой декабрь был прошлым; выручка, посчитанная по угаданному году,
   хуже, чем никакой. Строки несутся как есть, окно — «последние N записей».
2. **Вид события берётся из контрагента, а не из знака.** `+`/`-` говорит, что
   предмет пришёл или ушёл, и не отличает продажу от выставления — забирают
   предмет обе. Человек на той стороне есть только у продажи и покупки. Читай мы
   знак, каждый когда-либо выставленный лот пошёл бы в выручку.

Это единственный парсер в проекте, написанный раньше замера. Поэтому вместе с
ним лежит `tools/probe-history.ts`: одна страница живых строк со знаком,
контрагентом, вердиктом и сырой разметкой каждого найденного вида. Запустить на
живом `/market` — и догадки станут фактами или исправятся.

**Табы существовали три месяца и ни разу не показались.** Матчеры фич взаимно
исключают друг друга, так что `sections.size < 2` было истинно всегда, и вся
машинерия вкладок работала вхолостую. «Продажи» встают на `/market` рядом с
репрайсом — первая страница в истории проекта с двумя вкладками. Заодно
выяснилось, что ярлык «Репрайс» никто никогда не читал: рядом с «Продажами» это
жаргон против человеческого слова, стало «Мои лоты».

**«Переставить» получил дорогу назад.** Нажать ещё раз — не отмена: репрайсер
целится в рынок, и второй проход встанет туда, где рынок сейчас. Цены «до» уже
лежали в плане (`ourBuyer`), так что запись стоит ноль бухгалтерии во время
прохода и пишется после него, только по тем лотам, которые правда сдвинулись.

Сложность в другом: `sellitem` не возвращает id лота, а лот, ждущий Steam Guard,
на витрине ещё не стоит. Значит, адресовать новые лоты нельзя — восстановление
перечитывает страницу, сопоставляет наши текущие лоты предмета с запомненными
ценами по порядку цен и двигает их. Чего не нашлось, считается вслух: половина
партии, восстановленная молча, хуже, чем число, сказавшее, что это половина.
Цикл записи вынесен в `runMoves(todo, verb)` и теперь общий — восстановление
через вторую реализацию было бы вторым набором тех же багов.

**`loadInventory` умел обходить весь инвентарь и использовался только как
аварийный дочитыватель безымянных плиток.** «Оценить всё» читает инвентарь по
выбранной игре целиком, называет число разных предметов и верхнюю оценку времени
и спрашивает — тот же ритуал, что уже стоит перед историей цен. Чтение дёшево,
цены — нет, поэтому дорогая половина проговаривается.

Мелочи того же разбора: «Стоп» на пяти вкладках был нарисован и заблокирован с
первой отрисовки — теперь его нет, пока нечего останавливать. «Снять» стояло
плечом к плечу с «Переставить», два необратимых массовых действия в ширину мыши
друг от друга, различимые только цветом; вынесено на свою строку и оставлено там
маленьким — полноширинная красная кнопка под полноширинной зелёной читается как
главная из двух. В попапе «Как это настроено» было десять строк
(«Запросов цен разом: 2», «Точный минимум книги: спрашиваем»); осталось три —
пауза, подрезание, потолок покупки, — остальное в README, где рядом лежат
причины.

755 тестов. `content.js` — 209,5 КБ, `content.css` — 13,0 КБ. Живьём по-прежнему
ничего не запускалось: вёрстка проверена статическим превью на собранном CSS.


## 2.49.0 — «Оценить всё» спрашивает страницу, а не адрес

Первая правка по живому запуску 2.48.0. Обе поломки — из одного корня: панель
верила URL там, где страница говорит прямо.

`#730_2` Steam дописывает только после *клика* по игре. Инвентарь, открытый
ссылкой или восстановленный вкладкой, фрагмента не имеет вовсе — и «Оценить всё»
отвечало «сначала выбери игру», когда эта игра нарисована на экране за панелью.
Steam оборачивает каждый загруженный инвентарь в
`#inventory_{steamid}_{appid}_{contextid}` и прячет все, кроме открытого. Этот
один id отвечает на оба вопроса сразу: какая игра открыта и чей это рюкзак.
Теперь фрагмент — лишь первый из четырёх источников, остальные читают то, что
на экране, и ответ — список: у одной игры бывает несколько инвентарей (CS2
рисует контексты 2 и 16 рядом), а «весь инвентарь по этой игре» значит все.

Тот же id — второе утверждение о владельце. `g_rgProfileData` — глобал шапки
профиля, и страница может дорисовать сетку без него; на vanity-адресе URL тоже
молчит, и 2.48.0 в этом случае показывал «не могу подтвердить, что инвентарь
твой» — над своим же рюкзаком, с убранными кнопками записи. Ничего при этом не
разрешается: `mine` по-прежнему требует совпадения с залогиненным id, так что
неверный разбор разметки может только промолчать.

«Выставить» было третьим в ряду из трёх, серым до конца скана и не говорило
цены — а цену выбирает настройка, у которой больше нет ручки, так что назвать её
было некому. Кнопка ушла на свою строку, приходит вместе с тем, что можно
выставить, и берёт ширину: «Выставить 34 — по минимуму рынка».

Заодно: `shownText` в тестах владельца спрашивал `textContent` у корня панели,
который повторяет всё, что под ним, включая скрытое, — то есть отвечал «да» на
любой вопрос, и его проверки «этого текста нет» не могли упасть. Теперь листья.

769 тестов. `content.js` — 211,2 КБ, `content.css` — 13,1 КБ. Живьём запускалось
только то, что прислал владелец: два экрана статуса. Вёрстка — статическое
превью на собранном CSS.


## 2.49.1 — две поломки по живому: null-полка и утроенные строки

**`Cannot read properties of null`.** `/inventory/{id}/753/1` — полка подарков,
которую `g_rgAppContextData` перечисляет с ненулевым счётчиком, как всякую
другую, — отвечает голым `null`. Раньше это не всплывало: «Оценить всё» читало
ровно один контекст из фрагмента адреса. С 2.49.0 оно читает все контексты
открытой игры как набор — и одна полка, на которой ничего ценного и нет,
убивала весь проход. Пустой ответ — это пустая полка, а не отказ; плюс один
непрочитанный контекст больше не роняет остальные (бан и «стоп» — по-прежнему
роняют, они про аккаунт, а не про эту полку).

**«Продано 0», 200 строк из 300 не разобраны.** Steam строит id ячеек *внутри*
строки на её же основе: строка — `history_row_{listing}_{event}`, а её потроха —
`..._name`, `..._price`. Регулярка без `$` совпадала со строкой и обоими
детьми, так что каждое событие считалось трижды: один раз верно и дважды пустой
скорлупой. Все суммы считались по списку, две трети которого — мебель. Это тот
самый разбор, написанный раньше замера, и первый же живой запуск его и поймал —
ровно тем счётчиком «не разобрал», который для этого и рисовался.

Заодно расширен признак «на той стороне человек»: ссылка на профиль была
единственным, что читалось, а Steam рисует собеседника ещё аватаркой и
`data-miniprofile`. У выставления и снятия называть некого, так что отсутствие
всех трёх — по-прежнему то, что делает строку нашей собственной. Осталось ли
«продано 0» после этого — покажет `npm run probe`: проба теперь печатает целиком
ту ячейку, на которой держится вся классификация, и считает вложенные id.

776 тестов. `content.js` — 211,4 КБ, `content.css` — 13,1 КБ.


## 2.49.2 — разбор истории показывает улику, а не только вердикт

Утроение строк починено (2.49.1), и осталось голое: сто записей, все «−»,
продаж ноль. «−» стоит и у продажи, и у выставления — предмет уносят обе, — так
что сотня минусов подряд как раз и означает «продажи и выставления вперемешку».
Вывод «продано 0» по такой странице почти наверняка неверен.

Чего не хватает — измерения: различает эти строки человек на той стороне, и
неизвестно, есть ли он вообще в том, что отдаёт `/render/`. Изнутри разбора
«тихий аккаунт» и «смотрю не в ту ячейку» неотличимы, поэтому разбор перестал
выбирать между ними молча. `HistoryEvent.who` несёт дословный текст ячейки
`.market_listing_whoactedwith` (или `null`, если ячейки нет вовсе — это другая
поломка, и её надо отличать), а сводка собирает до четырёх **различных**
вариантов. Если среди сотни строк есть настоящие продажи, их ячейка скажет не
то же, что у выставлений, и это всплывёт в списке само.

Заголовок при нуле опознанных сторон больше не утверждает: «Продано 0 — но
второй стороны нет ни в одной из 100 записей», жёлтым, а под «подробнее» —
что именно написано в той ячейке. Панель, которая выносит вердикт по сотне
строк, должна уметь предъявить основание.

779 тестов.


## 2.50.0 — релиз: инвентарь читает то, что нарисовано, а история признаётся, чего не видит

Сборка всего, что накопилось после 2.47.0. Общая нить одна: панель перестала верить
адресной строке там, где страница называет факт прямо.

**Инвентарь.** «Оценить всё» спрашивал только `location.hash` — а Steam пишет туда `#730_2`
лишь после того, как игру *кликнули*. Свежеоткрытый инвентарь с нарисованной сеткой получал
«сначала выбери игру». Но сетка сама себя называет: `id="inventory_{steamid}_{appid}_{contextid}"`.
`inventoriesOnPage` разбирает эти id, `activeTargets` складывает из них цель (хэш → открытая
сетка → плитки → таблица контекстов), а `ownerFromPage` заодно отвечает, чей это рюкзак, когда
`g_rgProfileData` молчит. Побочно: раз игра берётся целиком, читаются *все* её контексты —
и вот тут вылезло, что Steam отвечает на неоткрываемую полку (`753/1`, подарки) голым `null`.
`mergeInventory` теперь склеивает ничто из ничего, а `loadInventory` на `null` останавливается,
а не падает с «Cannot read properties of null».

**Продажа из инвентаря.** После оценки было нечем воспользоваться. Появилась отдельная строка
`.stw-actions-sell`: одна широкая зелёная кнопка, которая показывается только когда есть что
выставлять, и называет правило цены прямо в подписи — «Выставить 12 — по минимуму рынка».
Кнопка ничего не делает молча: подтверждение остаётся.

**История продаж.** Селектор строк был неякорным, и `history_row_A_B_name` — собственный
ребёнок строки — считался строкой. Каждое событие читалось трижды, отсюда «прочитано 300,
не разобрал 200». Якорь `/^history_row_(\d+)_(\d+)$/` это закрыл.

Осталась честная неизвестность: продажу от выставления отличает только человек на той стороне,
и его не видно ни в одной записи. Разбор больше не выдаёт догадку за факт — `HistoryEvent.who`
несёт дословный текст `.market_listing_whoactedwith` (или `null`, если ячейки нет вовсе), сводка
собирает до четырёх различных вариантов, а заголовок при нуле сторон становится жёлтым
«Продано 0 — но второй стороны нет ни в одной из N записей». Следующий заход начнётся с
измерения, а не с третьей догадки.

779 тестов. `content.js` — 213,1 КБ, `content.css` — 13,1 КБ.
